...
ui <- fluidPage(inputPanel(
  selectInput("data", "", choices = sort(Master$Pitcher), selected = ""),
  dateRangeInput("date","", format = "mm-dd-yyyy", start = "09-27-2018", end = Sys.Date()),
  checkboxGroupInput("batter", label = ("Batter Side"), choices = unique(Master$BatterSide), selected = c("Right", "Left"), inline = TRUE)),
  mainPanel(dataTableOutput("outtable"), plotlyOutput("outplot")))
...
}, options = list(searching = FALSE, paging = FALSE, order = list(1, 'desc'), dom = 't'))
...
sc <-  ggplot(bases, aes(x = xa, y = ya)) + 
  geom_path() + 
  geom_point(data = filter_df, x = filter_df$x, y = filter_df$y, 
             aes(color = factor(filter_df$PlayResult), text = paste("", PlayResult, "<br> Exit Speed: ", ExitSpeed, "<br> Distance: ", Distance, 
