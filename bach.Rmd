...
sz <- data.frame(x, z)
aa <- data.frame(aggregate(RelSpeed ~ TaggedPitchType, data = Pitcher, mean))
bb <- data.frame(aggregate(RelSpeed ~ TaggedPitchType, data = Pitcher, range))
a1 <- aa[1,2]
a2 <- aa[2,2]
a3 <- aa[3,2]
b1 <- bb[2,2]
...
Throws: `r az` | | 
-|-|
Fastball Range: | `r round(b1, digits = 2)` mph
Fastball Avg: | `r round(a2, digits = 2)` mph
Change Up Avg: | `r round(a1, digits = 2)` mph
Slider Avg: | `r round(a3, digits = 2)` mph
