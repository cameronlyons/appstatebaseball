...
<center>
```{r Spray Chart, eval=FALSE, include=FALSE}
...
```{r Ninety Plus, eval=FALSE, include=FALSE}
NinetyPlus <- hitter %>%
  filter(ExitSpeed >= 90)
np <- ggplot() +
  geom_path(data = sz, aes(x = x, y = y)) +
...
```{r Fastball Takes, eval=FALSE, include=FALSE}
